export default class BitArray640 extends DataView {
  ...
  // And of this and bar. Example: 1100 & 1001 = 1000
    let len = Math.min(this.buffer.byteLength,bar.buffer.byteLength),
        res = inPlace ? this : new BitArray64(len * 8);
    for (var i = 0; i < len; i += 4) res.setBigUint64(i,this.getBigUint64(i) & bar.getBigUint64(i));
    return res;
  }
  ...
  // Returns true if any of the bits in the BitArray are set. If returns false then all bits are 0
    let len = this.buffer.byteLength,
        res = true;
    for (var i = 0; res && i < len; i += 4) res = this.getBigUint64(i) === 0;
    return !res;
  }

  at(i){
  // Fetches the value at the given index
    return this.getBigUint64(i / 64) & (1 << (i & 7)) ? 1 : 0;
  }

  clear(){
  // Resets the BitArray in place
    for (let i = 0, len = this.buffer.byteLength; i < len; i += 4) this.setBigUint64(i,0);
  }
  ...
  // Returns true if all bits in the BitArray are set.
    let len = this.buffer.byteLength,
        res = true;
    for (var i = 0; res && i < len; i += 4) res = this.getBigUint64(i) === 0xffffffff;
    return res;
  }

  fill(){
  // Sets the BitArray in place
    for (let i = 0, len = this.buffer.byteLength; i < len; i += 4) this.setBigUint64(i,0xffffffff);
  }
  ...
  // Flips all the bits in this buffer. Example: 1100 = 0011
    let len = this.buffer.byteLength,
    res = inPlace ? this : new BitArray64(len * 8);
    for (var i = 0; i < len; i += 4) res.setBigUint64(i,~(this.getBigUint64(i) >>> 0));
    return res;
  }
  ...
  reset(i){
    ...
    this.setBigUint64(i / 64, this.getBigUint64(i / 64) & ~(1 << (i & 7)));
  }
  ...
  // The default argument values instantiate a clone.
    ...
    return new BitArray64(this.buffer.slice(a, b));
  }

  toggle(i){
	// Flips the value at the given index
    this.setBigUint64(i / 64, this.getBigUint64(i / 64) ^ (1 << (n & 7)));
  }

  // For efficiency maps this.buffer to an BigUint64Array and byte by byte reverses the rank of bits and stringifies by
  // reducing. However stringifying a huge BitArray is meaningless. Perhaps limiting the string size to 128 is reasonable.
  toString(){
    return new BigUint64Array(this.buffer).reduce((p,c) => p + ((BigInt(c)* 0x0202020202n & 0x010884422010n) % 1023n).toString(2).padStart(8,"0"),"");
  }
  ...
  clone() {
    ...
    let result = new BitArray64(this.buffer.byteLength);
    for (let i = 0; i < this.buffer.byteLength/8; i++) {
      result.setBigUint64(i, this.getBigUint64(i));
    }
    return result;
  }
}