export default class BitArray32 extends DataView {
  ...
  // And of this and bar. Example: 1100 & 1001 = 1000
    let len = Math.min(this.buffer.byteLength,bar.buffer.byteLength),
        res = inPlace ? this : new BitArray32(len * 8);
  ...
  // Fetches the value at the given index
    return this.getUint32(i / 8) & (1 << (i & 7)) ? 1 : 0;
  }
  ...
  reset(i){
    ...
    this.setUint32(i / 8, this.getUint32(i / 8) & ~(1 << (i & 7)));
  }
  ...
  // The default argument values instantiate a clone.
    ...
    return new BitArray32(this.buffer.slice(a, b));
  }

  toggle(i){
	// Flips the value at the given index
    this.setUint32(i / 8, this.getUint32(i / 8) ^ (1 << (n & 7)));
  }

  // For efficiency maps this.buffer to an Uint32Array and byte by byte reverses the rank of bits and stringifies by
  // reducing. However stringifying a huge BitArray is meaningless. Perhaps limiting the string size to 128 is reasonable.
  toString(){
    return new Uint32Array(this.buffer).reduce((p,c) => p + ((BigInt(c)* 0x0202020202n & 0x010884422010n) % 1023n).toString(2).padStart(8,"0"),"");
  }
  ...
  clone() {
    ...
    let result = new BitArray32(this.buffer.byteLength);
    for (let i = 0; i < this.buffer.byteLength/8; i++) {
      result.setUint32(i, this.getUint32(i));
    }
    return result;
  }
}