export default class BitArray16 extends DataView {
  ...
  // And of this and bar. Example: 1100 & 1001 = 1000
    let len = Math.min(this.buffer.byteLength,bar.buffer.byteLength),
        res = inPlace ? this : new BitArray16(len * 8);
    for (var i = 0; i < len; i += 4) res.setUint16(i,this.getUint16(i) & bar.getUint16(i));
    return res;
  }
  ...
  // Returns true if any of the bits in the BitArray are set. If returns false then all bits are 0
    let len = this.buffer.byteLength,
        res = true;
    for (var i = 0; res && i < len; i += 4) res = this.getUint16(i) === 0;
    return !res;
  }

  at(i){
  // Fetches the value at the given index
    return this.getUint16(i / 16) & (1 << (i & 7)) ? 1 : 0;
  }

  clear(){
  // Resets the BitArray in place
    for (let i = 0, len = this.buffer.byteLength; i < len; i += 4) this.setUint16(i,0);
  }
  ...
  // Returns true if all bits in the BitArray are set.
    let len = this.buffer.byteLength,
        res = true;
    for (var i = 0; res && i < len; i += 4) res = this.getUint16(i) === 0xffffffff;
    return res;
  }

  fill(){
  // Sets the BitArray in place
    for (let i = 0, len = this.buffer.byteLength; i < len; i += 4) this.setUint16(i,0xffffffff);
  }
  ...
  // Flips all the bits in this buffer. Example: 1100 = 0011
    let len = this.buffer.byteLength,
    res = inPlace ? this : new BitArray16(len * 8);
    for (var i = 0; i < len; i += 4) res.setUint16(i,~(this.getUint16(i) >>> 0));
    return res;
  }
  ...
  reset(i){
    ...
    this.setUint16(i / 16, this.getUint16(i / 16) & ~(1 << (i & 7)));
  }
  ...
  // The default argument values instantiate a clone.
    ...
    return new BitArray16(this.buffer.slice(a, b));
  }

  toggle(i){
	// Flips the value at the given index
    this.setUint16(i / 16, this.getUint16(i / 16) ^ (1 << (n & 7)));
  }

  // For efficiency maps this.buffer to an Uint16Array and byte by byte reverses the rank of bits and stringifies by
  // reducing. However stringifying a huge BitArray is meaningless. Perhaps limiting the string size to 128 is reasonable.
  toString(){
    return new Uint16Array(this.buffer).reduce((p,c) => p + ((BigInt(c)* 0x0202020202n & 0x010884422010n) % 1023n).toString(2).padStart(8,"0"),"");
  }
  ...
  clone() {
    ...
    let result = new BitArray16(this.buffer.byteLength);
    for (let i = 0; i < this.buffer.byteLength/16; i++) {
      result.setUint16(i, this.getUint16(i));
    }
    return result;
  }
}